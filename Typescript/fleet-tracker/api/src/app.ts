...
app.post("/telemetry", (req, res) => {
  ...
  const result = ingestPings(pings);
  res.status(202).json(result);
});

app.get("/vehicles/:id/trips", (req, res) => {
  const vehicleId = req.params.id;
  const trips = reconstructTrips(getPings(vehicleId));
  res.json({ vehicle_id: vehicleId, trips });
});

app.get("/vehicles/:id/utilization", (req, res) => {
  const vehicleId = req.params.id;
  ...
  res.json(computeUtilization(vehicleId, from, to));
});

app.get("/fleet/summary", (_req, res) => {
